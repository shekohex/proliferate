...
export class OneWayWebSocket<TData = unknown> implements OneWayWebSocketApi<TData> {
	...
	addEventListener<TEvent extends WebSocketEventType>(
		...
		const looseCallback = callback as OneWayEventCallback<TData, WebSocketEventType>;
		...
		const wrapped = (event: MessageEvent<string>): void => {
			const messageCallback = looseCallback as OneWayEventCallback<TData, "message">;
